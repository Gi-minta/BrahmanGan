...
public interface IVacunacionService
{
    ...
    Task<DesparasitacionResponse> AplicarDesparasitacionAsync(AplicarDesparasitacionRequest req, CancellationToken ct = default);
    Task<IReadOnlyList<DesparasitacionResponse>> ListarDesparasitacionesPorAnimalAsync(int idAnimal, CancellationToken ct = default);
}
